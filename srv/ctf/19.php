<?php
	...
	*/
	...
	if(isset($_POST["mode"])){
		...
		$db = new PDO("mysql:host=127.0.0.1;dbname=p19", "p19");
		...
		if($mode === "Login"){
			...
			if($row = $ps->fetch()){
				...
				if($row["Privileged"]){
					$message .= "<br><br>You are privileged user.<br>FLAG is FLAG_392b3d013072d36a7f1e1d4f3510a44f593cf6e5";
				}else{
			...
			if($ps->fetch()){
				$ps = $db->prepare("UPDATE Users SET Password = ? WHERE Username = ?");
				$ps->execute([$newpass, $user]);
