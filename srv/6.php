<?php
	...
	if(isset($_POST["mode"])){
		...
		// DBに接続
		$db = new PDO("mysql:host=127.0.0.1;dbname=db2", "root");
		...
		if($mode === "Login"){
			...
			// 問い合わせ
			$ps = $db->prepare("SELECT * FROM account WHERE user = ?");
			$ps->execute([$user]);
			$row = $ps->fetch();
			...
			if($row){
				// パスワードの変更を行う
				$ps = $db->prepare("UPDATE account SET pass = ? WHERE user = ?");
				$ps->execute([$newpass, $row["user"]]);
